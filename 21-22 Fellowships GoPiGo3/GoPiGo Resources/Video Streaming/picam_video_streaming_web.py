"""
    Name: video_streaming_picamera_rover.py
    Author: 
    Created: 10/21/21
...
import logging
import socketserver              # Create server socket for client connection
import socket                    # For finding local IP address
from threading import Condition
from http import server          # For creating a local web server
...
<html>
<head>
<titleMARS Rover Streaming Video</title>
</head>
<body>
<h1>GoPiGo Streaming Video</h1>
<img src="stream.mjpg" width="640" height="720" />
</body>
</html>
"""
...
def main():
    global output
    global address
    with picamera.PiCamera(resolution='640x720', framerate=24) as camera:
        # Add rotation = 90 as camera is mounted sideways
        camera.rotation = 90
        ...
        print(f"Streaming started at: http://{get_ip_address()}:8000")
        try:
